...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(
    chatId,
    "Welcome to MovieSuggester! 🎬 Use the button below to get started.",
    {
      reply_markup: {
        inline_keyboard: [
  ...
  try {
    ...
    if (movies.length > 0) {
      ...
      if (userSession.recommendations.length > 0) {
        ...
        bot.sendMessage(
          chatId,
          "That's all the recommendations I have for now. Would you like to start over?",
          {
            reply_markup: {
              inline_keyboard: [
                [{ text: "Start Over", callback_data: "recommend" }],
      ...
      bot.sendMessage(
        chatId,
        `Sorry, I couldn't find any ${userSession.genre} movies in ${language}. Would you like to try again?`,
        {
          reply_markup: {
            inline_keyboard: [
    ...
    bot.sendMessage(
      chatId,
      "Sorry, something went wrong while fetching recommendations. Please try again.",
      {
        reply_markup: {
          inline_keyboard: [
            [{ text: "Try Again", callback_data: "recommend" }],
...
async function handleGenerateMore(chatId, userSession) {
  if (!userSession) {
    bot.sendMessage(
      chatId,
      "It seems that your session has expired. Please start over to get new recommendations.",
      {
        reply_markup: {
          inline_keyboard: [
            [{ text: "Start Over", callback_data: "recommend" }],
  ...
  if (userSession.recommendations.length > 0) {
    ...
    bot.sendMessage(
      chatId,
      "No more recommendations available. Would you like to start over?",
      {
        reply_markup: {
          inline_keyboard: [
            [{ text: "Start Over", callback_data: "recommend" }],
